...
from core.video_capture import NamedBox, OpenCvScreen, OpenCvVideoCapture
...
class FaceRecognizer(BaseMiddleware):
    ...
    def __init__(
        ...
        self._recognizer.read(
            os.path.join(
                settings.BASE_DIR + "/data/training",
                f"{self._recognizer_type.value[1]}.yml",
            ),
        )
        self._faces = []
    ...
    def _process(self, frame):
        ...
        for x, y, w, h in self._faces:
            ...
            cv.putText(
                frame,
                f"{self._recognizer_type.value[1]}",
                (x, y + h + 10),
                cv.FONT_HERSHEY_PLAIN,
                0.5,
                self._color,

...
from core.utils import GetScreen, Point
...
class NamedBox:
    ...
    def _paint_indicator_box(self, start, end):
        ...
        triangle = np.array(
            [point_1.as_tuple(), point_2.as_tuple(), point_3.as_tuple()],
        )
    ...
    def show(self):
        ...
        end_point = Point(
            start_point.x + text_width + 18,
            start_point.y + text_height + 20,
        )
        text_point = Point(start_point.x + 10, start_point.y + 22)
        self._paint_box(start_point, end_point, text_point)._paint_indicator_box(
            start_point,
            end_point,
        )
        return self
...
class OpenCvCamCaptureByRtsp(AbstractOpenCvVideoCapture):
    ...
    def init_capture(self):
        ...
        if not result:
            raise Exception(f"Impossible to connect with: {self._url}")
        return result
